...
def get_loggers(log_dir, conf):
    ...
    packet_logger = setup_logger(
        'simulation.packet', os.path.join(log_dir, 'packet_log.csv'))
    packet_logger.info(StructuredMessage(metadata=("Type", "CurrentTime", "ClientID", "PacketID", "PacketType", "MessageID", "PacketTimeQueued",
                       "PacketTimeSent", "PacketTimeDelivered", "TotalFragments", "PrOthers", "PrSenderA", "PrSenderB", "RealSenderLabel", "Route", "PoolSizes")))

    message_logger = setup_logger(
        'simulation.messages', os.path.join(log_dir, 'message_log.csv'))
    message_logger.info(StructuredMessage(metadata=("Type", "CurrentTime", "ClientID", "MessageID", "NumPackets",
                        "MsgTimeQueued", "MsgTimeSent", "MsgTimeDelivered", "MsgTransitTime", "MsgSize", "MsgRealSender")))

    entropy_logger = setup_logger(
        'simulation.mix', os.path.join(log_dir, 'last_mix_entropy.csv'))
    entropy_logger.info(StructuredMessage(metadata=tuple(
        ['Entropy'+str(x) for x in range(int(conf["misc"]["num_target_packets"]))])))
...
def run_p2p(env, conf, net, loggers):
    ...
    print("> Running the system for %s ticks to prepare it for measurment." %
          (conf["phases"]["burnin"]))
    ...
    # Run until the stop_sim_event is triggered.
    ...
    print("> Total Simulation Time [in ticks]: " +
          str(time_finished-time_started) + "---------")
    print("> Total Simulation Time [in unix time]: " +
          str(time_finished_unix-time_started_unix) + "---------")

    flush_logs(loggers)

    global throughput
    throughput = float(env.total_messages_received) / \
        float(time_finished-time_started)

    mixthroughputs = []
    for p in net.peers:
        mixthroughputs.append(float(p.pkts_sent) /
                              float(time_finished-time_started))

    print("Network throughput %f / second: " % throughput)
    print("Average mix throughput %f / second, with std: %f" %
          (np.mean(mixthroughputs), np.std(mixthroughputs)))
...
def run_client_server(env, conf, net, loggers):
    ...
    time_started = env.now
    # Unix time start (dd/mm/yyyy, hh:ss:..)
    time_started_unix = datetime.datetime.now()
    ...
    for m in net.mixnodes:
        mixthroughputs.append(float(m.pkts_sent) /
                              float(time_finished-time_started))

    print("Total number of packets which went through the network: ",
          float(env.total_messages_received))
    print("Network throughput %f / second: " % throughput)
    print("Average mix throughput %f / second, with std: %f" %
          (np.mean(mixthroughputs), np.std(mixthroughputs)))
...
def run(exp_dir, conf_file=None, conf_dic=None):
    ...
    # time after which we should terminate the target senders from sending
    limittime = conf["phases"]["burnin"] + conf["phases"]["execution"]
    simtime = conf["phases"]["burnin"] + conf["phases"]["execution"] + \
        conf["phases"]["cooldown"]  # time after which the whole simulator stops
